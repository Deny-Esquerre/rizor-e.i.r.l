...
export default function PersonnelPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setFormData({ first_name: "", last_name: "", dni: "", phone: "", address: "", area: "Producción", monthly_salary: 0 })
            ...
            <Table>
              <TableHeader className="bg-muted/50">
                <TableRow className="hover:bg-transparent">
                  <TableHead className="w-[30%] font-bold text-foreground py-3">Nombre Completo</TableHead>
                  <TableHead className="w-[15%] font-bold text-foreground py-3">Documento / Tel.</TableHead>
                  <TableHead className="w-[20%] font-bold text-foreground py-3">Área / Cargo</TableHead>
                  <TableHead className="w-[15%] text-right font-bold text-foreground py-3 whitespace-nowrap">Sueldo Mens.</TableHead>
              ...
              <TableBody>
                {loading ? (
                  Array.from({ length: 5 }).map((_, i) => (
                    <TableRow key={i}>
                      <TableCell colSpan={5}><div className="h-12 w-full animate-pulse rounded-lg bg-muted/40" /></TableCell>
                    </TableRow>
                  ))
                  ...
                  filteredMembers.map((member) => (
                    <TableRow key={member.id} className="group hover:bg-muted/30 transition-colors">
                      ...
                      <TableCell className="text-right py-3">
                        <span className="text-sm font-black text-foreground">S/ {Number(member.monthly_salary).toFixed(2)}</span>
                      </TableCell>
              ...
              {previewMember && (
                <div className="p-8 space-y-6">
                  ...
                  <div className="bg-card ring-1 ring-border/40 rounded-2xl p-5 flex justify-between items-center shadow-sm">
                    <div className="space-y-1">
                      <Label className="text-[10px] uppercase font-black text-primary/60 tracking-widest">Remuneración</Label>
                      <p className="text-2xl font-black text-emerald-600">S/ {Number(previewMember.monthly_salary).toFixed(2)}</p>
                    </div>
